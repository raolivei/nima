...
nima uses **Flux-First with Emergency Override** deployment strategy. This means:
...
This compares `nima/k8s/` with `pi-fleet/clusters/eldertree/nima/`
...
- `nima/k8s/` - Direct deployment manifests
- `pi-fleet/clusters/eldertree/nima/` - Flux-managed manifests

**After any changes**:

```bash
# Copy changes to pi-fleet
cp k8s/*.yaml ../pi-fleet/clusters/eldertree/nima/

# Validate sync
./scripts/validate-k8s-sync.sh

# Commit both locations
cd ../pi-fleet && git add . && git commit -m "Update nima manifests"
cd ../nima && git add . && git commit -m "Update k8s manifests"
```
...
kubectl logs -n nima -l app=nima-api
kubectl logs -n nima -l app=nima-frontend
```
...
# Manual sync
diff -r k8s/ ../pi-fleet/clusters/eldertree/nima/
```
